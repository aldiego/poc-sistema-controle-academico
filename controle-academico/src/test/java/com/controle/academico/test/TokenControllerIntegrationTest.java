package com.controle.academico.test;
...
import com.controle.academico.config.ControleAcademicoApplication;
import com.controle.academico.controller.TokenController;

@RunWith(SpringRunner.class)
@SpringBootTest(classes = {ControleAcademicoApplication.class, TokenController.class},
...
public class TokenControllerIntegrationTest {
  ...
  private List<String> retrieveTokens() throws Exception {
    return mapper.readValue(mockMvc.perform(get("/tokens")).andExpect(status().isOk()).andReturn()
        .getResponse().getContentAsString(), new TypeReference<List<String>>() {});
  }
